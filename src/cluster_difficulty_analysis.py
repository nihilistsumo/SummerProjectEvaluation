...
def main():
    ...
    parser.add_argument('-out', '--output', help='Path to score output file')
    ...
    for pp_score_file in pp_score_files:
        ...
        all_clust_score[method] = pagewise_ari
    ...
    with open(outpath, 'w') as o:
        ...
        for p in pages:
            o.write(p+'\t')
        o.write('\n')
        for m in all_clust_score.keys():
            o.write(m+'\t')
            for p in pages:
                o.write('%.4f\t' % all_clust_score[m][p])
            o.write('\n')
